...
function Home() {
  ...
  const activeVocabulary = language === "ar" ? vocabulary : englishVocabulary;
  const activeLetters = language === "ar" ? letters : englishLetters;
  ...
  const switchLanguage = (next: "en" | "ar") => { setLanguage(next); setQuizIndex(0); setSelectedAnswer(null); setCompleted(false); setCategory(next === "ar" ? "الكل" : "All"); setSelectedLetter(next === "ar" ? letters[0] : englishLetters[0]); };
  ...
  return (
    <div dir={language === "ar" ? "rtl" : "ltr"} className="min-h-screen overflow-x-hidden bg-paper text-ink">
      <header className="sticky top-0 z-40 border-b border-ink/10 bg-paper/90 backdrop-blur-xl"><div className="container flex h-[74px] items-center justify-between gap-6">
        <button className="flex items-center gap-3 text-right" onClick={() => scrollTo("top", "Home")} aria-label="Back to home"><span className="brand-mark"><span>ع</span></span><span><span className="block font-display text-lg font-bold leading-none text-ink">Learn Arabic</span><span className="mt-1 block text-[10px] font-bold uppercase tracking-[0.22em] text-ink/45" dir="ltr">{language === "ar" ? "العربية · يوميًا" : "ARABIC · DAILY"}</span></span></button>
